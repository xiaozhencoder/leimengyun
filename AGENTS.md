...
- **Seed scripts**: Run from `apps/server` with `npx ts-node --compiler-options '{"module":"CommonJS"}' prisma/seed-community.ts` or `prisma/seed-questionnaire.ts`. The questionnaire seed is idempotent (deletes existing system templates before re-creating). If FK constraint errors occur, delete `questionnaire_responses` and `questionnaire_assignments` tables first.
- **Questionnaire API shapes**: The `/questionnaire/assignments/received` endpoint returns flat fields (`templateTitle`, `doctorName`, `questionCount`, `estimatedTime`, `totalScore`, `templateTotalScore`), while `/questionnaire/assignments/:id/result` nests data under `response` (`response.totalScore`, `response.dimensionScores`, `response.answers`) and `template` (`template.totalScore`, `template.questions`). The `level` object contains `level`, `percentage`, and `message`.
